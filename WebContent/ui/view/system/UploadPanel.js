 ...
 * @since 2013-10-29
 */
Ext.define('zft.view.system.UploadPanel',{
	...
	columns : [
        ...
        {text: '进度', width: 130,dataIndex: 'percent',renderer:function(v){ 
			var stml =
				'<div>'+
					'<div style="border:1px solid #5EDF61;height:15px;width:115px;float:left;">'+		
						'<div style="float:left;background:#5EDF61;width:'+v+'%;height:13px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + v + '%</div>'+
					'</div>'+'</div>';
			return stml;
        }},
        {text: '状态', width: 80,dataIndex: 'status',align: 'center',renderer:function(v){
			var status;
			if(v==-1){
				status = "等待上传";
			}else if(v==-2){
		...
		}},
        {
            ...
            flex : 1,
            items: [{
                icon: Ext.contextPath + '/ext-4.2/resources/images/delete.gif',
                tooltip: '删除',
	...
	file_queue_limit : 0,
	post_params : {},
	flash_url : Ext.contextPath + '/ext-4.2/upload/swfupload.swf',
	flash9_url : Ext.contextPath + '/ext-4.2/upload/swfupload_fp9.swf',
	...
	getSWFConfig : function(btn){
		...
		return {
			...
			upload_url: me.upload_url+";jsessionid="+sessionId,
	...
	upload_start_handler : function(file){
		...
		if(file.type == '.xls'){
			this.setFilePostName('xlsFile');
		}else{
			this.setFilePostName('imgFile');
		}
	},
	...
	upload_success_handler : function(file, serverData, responseReceived){
		...
		if(Ext.JSON.decode(serverData).success){			
			...
			showErrorMsg(Ext.JSON.decode(serverData).msg,this);
			rec.set('percent', 0);
