...
public class T10301Action extends BaseAction {
	...
	private OSCache cache = CacheFactory.getInstance();
	private String str81229;
	...
	private void reportNoInfo() {
		
		List<Object> list = commQueryDAO.findBySQLQuery("select report_no from tbl_report_inf");
		StringBuffer sb = new StringBuffer();
		for(Object o : list){
			 sb.append(o.toString());
			 sb.append("|");
		}
		str81229 = sb.toString();
	}
	...
	private String add() {
		...
		for(int i = 0; i < len; i++) {
			...
			String[] values = jsonBean.getJSONDataAt(i).getString("valueId").split("_");
			Integer valueId = Integer.parseInt(values[0]);
			String roleFlag = values[1];
			tblRoleFuncMapPK.setValueId(valueId);
			tblRoleFuncMap.setRoleFlag(roleFlag);
			...
			//-----------------------------
			if(str81229.contains(tblRoleFuncMapPK.getValueId().toString()) && flag81229) {  
				tblRoleFuncMapPK = new TblRoleFuncMapPK();
				tblRoleFuncMapPK.setKeyId(tblRoleInf.getRoleId());
				tblRoleFuncMapPK.setValueId(81229);
				
				tblRoleFuncMap = new TblRoleFuncMap();
				tblRoleFuncMap.setId(tblRoleFuncMapPK);
				tblRoleFuncMap.setRoleFlag("1");
				...
				tblRoleFuncMapPK = new TblRoleFuncMapPK();
				tblRoleFuncMapPK.setKeyId(tblRoleInf.getRoleId());
				tblRoleFuncMapPK.setValueId(81234);
	...
	private String delete() {
		String sql = "select * from tbl_opr_info where opr_degree like '%" + roleId + "%'";
	...
	public String edit() {
		...
		for(int i = 0; i < len; i++) {
			...
			tblRoleFuncMapPK = new TblRoleFuncMapPK();
			//update by lvcg 增加权限控制 20130822
			String[] values = jsonBean.getJSONDataAt(i).getString("valueId").split("_");
			Integer valueId = Integer.parseInt(values[0]);
			String roleFlag = values[1];
	...
	private boolean getReportFlag(String roleId){
		String sql = "select * from tbl_role_func_map where key_id='"+roleId+"' and value_id in('81229','81234')";
		List<Object[]> dataList = commQueryDAO.findBySQLQuery(sql);
		if(dataList.size() > 0) {
			return false;
		}
