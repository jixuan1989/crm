	...
	public String execute() {
		...
		try {
			...
			if(getTxnId() != null && getSubTxnId() != null) {
				if(Constants.SUCCESS_CODE.equals(rspCode)) {
				...
				}else if(Constants.ERROR_CODE_CUSTOMIZE.equals(rspCode)) {
					writeErrorMsg(Constants.ERROR_CODE_CUSTOMIZE);
					...
					tblTxnInfo.setTxnSta(TblTxnInfoConstants.TXN_STA_FAILURE); //登陆失败
		...
		//	}
			...
			if(getTxnId() != null) {
				...
				if(e.getMessage() != null){
					if(e.getMessage().length() > 512) {
						tblTxnInfo.setErrMsg(e.getMessage().substring(0,512));
					} else {
			...
			log.debug("交易失败，交易码：[ " + getTxnId() + "."  + getSubTxnId() + " ]，异常信息：[ " + e.getMessage() + " ]");
			try {
				if(e.getMessage() != null){
					writeErrorMsg("<b>错误：</b>"+e.getMessage());
				}else {
